...
function process_data(data)
{
	...
	for(i=1;i<8;i=i+1)
	{
		...
		//document.getElementById(id_tempe).innerHTML= "<TABLE BORDER=\"0\" align=\"center\"> <TR> <TD width=\"140px\" align=\"left\">"+ (p.root.data.tempe[i]-400)/10+"&deg;C</TD> <TD width=\"140px\" align=\"right\">57%</TD> </TR> </TABLE>";
		document.getElementById(id_tempe).innerHTML= lacrosse2int(p.root.data.tempe[i])+"&deg;C";
		if (p.root.data.hygro[i] != 255)
		{
			id_hygro="hygro_"+nom_affectations[i];
			document.getElementById(id_hygro).innerHTML=p.root.data.hygro[i]+"%";
		}
	}
	...
	for(i=0;i<2;i=i+1)
	{
		...
		/* version chauffage pour premiere version page (etat.htm) avec plan appart ****************************************
		...
		// version pour index.htm, températures seulement, pas de modif possible *******************************************
		id_chauf= "chauf_"+nom_affectations[p.root.config.chauffage[i].affectation];
...
/*		for(j=0;j<p.root.data.chauffage[i].nbRad;j=j+1)
	...
	// lumino
	id_lum= "lumino_salon";
	document.getElementById(id_lum).innerHTML="lum "+Math.round(p.root.data.lumino / 4096 * 100)+"%";
}
