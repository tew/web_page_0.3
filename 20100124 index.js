...
/*        def="Mode <strong>"+mode_chauf[p.root.config.chauffage[i].mode]+"</strong>"
        ...
        {
            // mode automatique, on précise le mode sélectionné actuellement
            def=def+"/"+mode_chauf[p.root.data.chauffage.zones[i].modeCrt];
        }
        def=def+" ("+lacrosse2int(p.root.data.chauffage.zones[i].consCrt)+"&deg;C)&nbsp;";
        for(j=0;j<p.root.data.chauffage.zones[i].nbRad;j=j+1)
        {
            if (p.root.data.chauffage.zones[i].radActif>j)
            {
                def=def+"&nbsp;<img style=\"vertical-align:bottom\" height=\"30px\" src=\"flamme.gif\">";
            }
            else
...
<div class="entry"> \
  <TABLE BORDER="0" align="center"> \
    <TR><TD width="220px" align="right">Mode fonctionnement :&nbsp</TD> \
        ...
        {
                content=content+'<option value="'+noAff+'">'+mode_chauf[noAff]+'</option>';
        }
